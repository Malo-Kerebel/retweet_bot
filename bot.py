...
def retweet(Search, Message):
    ...
    tweet_list = []
    # The 'recent' can be change to 'mixed' or to 'popular'
    ...
    try:
        i = 0
        found = False
        while not found and i < len(tweet_list):
            tweet = tweet_list[i]
            found = True
        ...
        # Given the amount of tweets the duplicate check shouldn't be necessary
        # but if you search for a really precise term it might be usefull
        if (duplicate_check(tweet.id) is False):
            try:
                # twitter.retweet(tweet.id) could also works,
                # the code below quotes the tweets instead
                # It quotes the tweet by recreating the tweet URL
        ...
        # For some reason sometimes the 0th element of tweet_list
        # is out of bounds and throw this error,
